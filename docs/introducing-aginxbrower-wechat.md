...
于是我们做了 **AginxBrower**——一个 **70MB** 的 Rust 服务端浏览器。
...
AginxBrower 的解法叫 **stealth 指纹伪装**：在 TLS 层把自己伪装成 Chrome（用 BoringSSL，和真 Chrome 同一个加密库），HTTP 头和 navigator 指纹也对齐成 macOS 版 Chrome。
...
你的服务器
├── Agent A ──┐
├── Agent B ──┼──► AginxBrower（70MB，单进程，HTTP 服务）
└── 脚本 C ──┘
```

一个实例，所有应用共享。不用每个 Agent 各拉一个浏览器。

接入方式更是简单到离谱。以我们自己的 Agent 运行时 OpenCarrier 为例，原本的 `web_fetch` 工具走 reqwest，要接入 AginxBrower **只需要一个环境变量**：

```bash
export AGINXBROWER_URL=http://127.0.0.1:8089
```

- **设了** → 遇到微信这种风控站，自动转给 AginxBrower 抓
- **没设** → 行为完全不变，随时可回退
- **AginxBrower 挂了** → 自动降级回 reqwest，系统不瘫
...
cd aginxbrower && cargo build --release --features stealth
...
GitHub 仓库：**github.com/yinnho/aginxbrower**

---

## 写在最后

AginxBrower 背后是一个很朴素的判断：
...
我们相信，**Agent 互联网应该像访问网站一样简单**。AginxBrower 是这块基础设施的一小块拼图——让每一个 Agent，都拥有一颗轻量的浏览器心脏。

下一篇，我们会拆解 AginxBrower 怎么用 stealth 指纹骗过微信风控的技术细节，**关注不迷路**。
