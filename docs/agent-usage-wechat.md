# 用 AginxBrower 抓取微信公众号文章（推荐方式）

## 什么时候用 AginxBrower 而不是公众号 OA API

抓微信公众号文章内容时，**优先用 AginxBrower，不要用 `mcp_wechat_oa_get_article`**：

- **OA API 路线**（`mcp_wechat_oa_get_article`）：只能抓**自己公众号**的已发布文章，需要 `article_id` + 登录凭证，且要绕一圈找 article_id。
- **AginxBrower 路线**：直接用文章 URL 抓，**任意公开公众号文章都能抓，免 cookie、免登录**。

**唯一规则**：只要拿到的是 `https://mp.weixin.qq.com/s/xxxxx` 这种文章链接，直接用 AginxBrower 抓全文，**不要**去找 article_id、不要调 OA API。

## 调用方法

向 AginxBrower 发 HTTP 请求（`POST /eval`），执行 JS 提取标题/作者/正文：
...
- `<AGINXBROWER_ADDR>`：AginxBrower 服务地址（同机部署为 `127.0.0.1:8089`，跨机为服务器 IP `106.75.32.216:8089`）。
- `wait_secs: 4`：等微信页面 JS 渲染完，必须给（微信正文是 JS 渲染的，不等会拿到空）。
- **不需要任何 cookie**：微信文章是公开页，AginxBrower 的 stealth 指纹伪装已足够绕过风控。
...
4. 不要对同一 URL 短时间内重复抓（AginxBrower 有 10 分钟缓存，命中缓存直接返回；但缓存外的高频请求可能触发风控）。
